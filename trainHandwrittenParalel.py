from model.AttentionWanaSit import CreateDualAttentionModelWSHW
...
def saveCheckpoint(modelToSave, fold, SER):
    modelToSave.save("checkpoints/handwritten/S2SA/modeldual.h5")
...
def save_vocabulary(fold):
    np.save("vocabulary/paralel/w2iglyphhw.npy", w2iglyph)
    np.save("vocabulary/paralel/i2wglyphhw.npy", i2wglyph)
    np.save("vocabulary/paralel/w2ipositionhw.npy", w2iposition)
    np.save("vocabulary/paralel/i2wpositionhw.npy", i2wposition)
...
if __name__ == "__main__":
    ...
    for i in range(1):
        ...
        RawX, RawYGlyph, RawYPos = loadData("dataHandwritten/B-59.850/")
        split = len(RawX)*0.2
        ...
        TestYGlyph = RawYGlyph[:splitIndex]
        ValidYGlyph = RawYGlyph[splitIndex:splitIndex*2]
        TrainYGlyph = RawYGlyph[splitIndex*2:]

        TestYPos = RawYPos[:splitIndex]
        ValidYPos = RawYPos[splitIndex:splitIndex*2]
        TrainYPos = RawYPos[splitIndex*2:]
        ...
        model = CreateDualAttentionModelWSHW(FEATURESPERFRAME, ALPHABETLENGTHGLYPH, ALPHABETLENGTHPOSITION)

        bestvalue = TrainLoop(model, TrainX, Y_TrainGlyph, Y_TrainPos, TestX, Y_TestGlyph, Y_TestPos, fold)

        modelToTest = load_model("checkpoints/handwritten/S2SA/modeldual.h5")
